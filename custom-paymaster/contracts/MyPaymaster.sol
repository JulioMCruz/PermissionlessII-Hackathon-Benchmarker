...
contract MyPaymaster is IPaymaster, Ownable {
    ...
    returns (bytes4 magic, bytes memory context)
  {
    ...
    if (paymasterInputSelector == IPaymasterFlow.approvalBased.selector) {
      ...
      uint256 ETHUSDCPrice = 16538539; //readDapi(ETHdAPIProxy);
      uint256 USDCUSDPrice = 10000000000000000000000000; //readDapi(USDCdAPIProxy);
        ...
        string.concat(
          string.concat(
            string.concat(
              string.concat(
                string.concat(
                  "Min paying allowance too low + provided: ",
                  Strings.toString(providedAllowance)
                ),
                " required: "
              ),
              Strings.toString(requiredERC20)
            ),
            " requiredETH: "
          ),
          Strings.toString(requiredETH)
        )
      );
